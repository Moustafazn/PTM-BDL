...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  STEP 11b — PTM-BDL Ablation Study (revised 2026-06-28)                     ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
...
║      no_glyco            — phospho channel only (drops glyco baseline+delta)║
║      glyco_only          — glyco channel only                                ║
...
from step11_train import (  # noqa: E402
    ResistanceDataset, collate_fn, FocalLoss,
    train_epoch, validate, build_model_from_cfg,
)
from src.models.multimodal_predictor import PROTEIN_ID_EGFR, PROTEIN_ID_ERBB2  # noqa: E402
...
# ══════════════════════════════════════════════════════════════════════════════
# Site labels (used by Part 2 IG bucketing)
# ══════════════════════════════════════════════════════════════════════════════
...
# Index 7 in phospho slots = Y1092 in EGFR / Y1221 in ERBB2 (both GRB2 docking).
GRB2_PHOSPHO_INDEX = 7
# Slot 8 in EGFR glyco = N528; slot 4 in ERBB2 glyco = N530 (homologous pair).
EGFR_N528_INDEX = 8
ERBB2_N530_INDEX = 4
...
ABLATION_CONFIGS = {
    ...
    "no_glyco": dict(
        label="Model E: No glyco",
        description="Phospho channel active, glyco channel zeroed.",
        color="#9467bd", use_typed_attention=True, data_mode="no_glyco",
    ),
    "glyco_only": dict(
        label="Model F: Glyco only",
        description="Glyco channel active, phospho channel zeroed.",
        color="#8c564b", use_typed_attention=True, data_mode="glyco_only",
    ),
...
#   no_glyco         = Model B (phospho-only PTM-BDL)
#   full             = Model C (phospho+glyco PTM-BDL, production)
#   no_typed_attention = Model D (architectural ablation: MLP in place of
#                                  typed self-attention)
# plus glyco_only as a sanity arm — keeps glyco only.
ABLATION_ORDER = [
    "no_ptm", "no_glyco", "glyco_only", "no_typed_attention", "full",
]
...
def run_ablation_study(device):
    ...
    # Channel-level gains
    glyco_marginal = (full_m.get("auroc", 0)
    ...
    print(f"    Phospho marginal             : {phospho_marginal:+.3f} "
          f"(full vs glyco-only)")
    print(f"    Glyco marginal               : {glyco_marginal:+.3f} "
    ...
    summary = {
        ...
        "glyco_marginal_auroc": round(glyco_marginal, 4),
...
def _run_ptm_bdl_ig(model, dataset, indices, df, n_steps: int = 20):
    ...
    Integrates along ALL 4 independent input channels simultaneously:
      ...
      - glyco_vector     (glyco baseline level, baseline=1.0 = WT)
      - delta_glyco_vector (drug-induced glyco change, baseline=0.0 = no drug)
    ...
    for idx in indices:
        ...
        actual_glyco = sample["glyco_vector"]
        actual_dphospho = sample["delta_ptm_vector"]
        actual_dglyco = sample["delta_glyco_vector"]
