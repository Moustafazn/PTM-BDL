...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  STEP 13 — PTM-BDL Explainability, Cross-Type Attention & Biological        ║
...
from src.models.multimodal_predictor import (  # noqa: E402
    PROTEIN_ID_EGFR, PROTEIN_ID_ERBB2, PTM_TYPE_NAMES,
)
from step11_train import (  # noqa: E402
    ResistanceDataset, build_model_from_cfg,
)
...
# glyco_vector simultaneously per sample, with baseline = ones (the WT-
...
def compute_ptm_bdl_ig(model, dataset, indices, n_steps: int = 30):
    ...
    for i, idx in enumerate(indices):
        ...
        actual_gl = sample["glyco_vector"]
        actual_dph = sample["delta_ptm_vector"]
        actual_dgl = sample["delta_glyco_vector"]
...
def compute_cross_type_attention(model, dataset, indices):
    ...
    for i, idx in enumerate(indices):
        ...
        attn = model.ptm_bdl.compute_attn_weights(
            ...
            sample["glyco_vector"].unsqueeze(0),
            sample["delta_glyco_vector"].unsqueeze(0),
            tp,
        )
