...
RAW_DIR = PROJECT_ROOT / cfg["paths"]["raw_data"] / "ccle"          # input: downloaded files
OUT_DIR = PROJECT_ROOT / cfg["paths"]["processed_data"] / "ccle"    # output: processed CSVs & FASTA
...
def extract_erbb2_mutations(mutations_path: Path, model_path: Path):
    ...
    if erbb2_chunks:
        ...
        if agg_dict:
            ...
            profiles.rename(columns={"ProteinChange": "erbb2_mutations", "mutation_class": "mutation_classes"}, inplace=True)
...
def build_erbb2_comprehensive_profiles():
    ...
    # Load GDSC ERBB2 breast cancer cell lines
