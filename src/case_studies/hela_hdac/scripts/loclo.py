...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  HeLa/HDAC — Leave-One-Cell-Line-Out (LOCLO) Generalization Test           ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  PURPOSE:                                                                    ║
║    Test generalization to UNSEEN cell lines by holding out entire tissue     ║
║    type groups. This is the "cell-blind" split required by the 2026 DRP     ║
║    review (Sada Del Real et al., Brief Bioinf):                             ║
...
║    With multi-cell-line data from GDSC (step06), we can now do TRUE         ║
║    cell-blind LOCLO. Cell lines are grouped by cancer type / tissue:        ║
║      • cervical, breast, lung_nsclc, leukemia, colorectal, etc.            ║
║    Holding out a tissue group tests: "Can the model predict HDAC            ║
║    inhibitor response for cancer types it has never seen?"                  ║
║                                                                              ║
║  BIOLOGICAL SIGNIFICANCE:                                                    ║
║    HDAC inhibitors show tissue-dependent efficacy:                           ║
║      • Hematological cancers often more sensitive than solid tumors          ║
║      • T-cell lymphoma approved for Romidepsin, not lung cancer             ║
║      • CUDC-101 triple inhibitor may work differently in EGFR+ tumors      ║
║    If the model generalizes across tissues, PTM biology is transferable.    ║
║    Ref: Marks & Xu, J Cell Biochem 2009 — tissue-dependent SAHA response   ║
║    Ref: Hartl et al., Cell Reports 2024 — HDAC inhibitor proteomics        ║
║                                                                              ║
║  INPUT:                                                                      ║
║    data/processed/hela_hdac/multimodal_dataset.csv (from step06)            ║
║                                                                              ║
║  OUTPUT:                                                                     ║
║    results/hela_hdac/loclo_results.json                                     ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import json
import time
...
def assign_tissue_groups(df: pd.DataFrame) -> np.ndarray:
    """
    Assign each sample to a tissue group for LOCLO.

    Uses the tissue_group column from step06 (which maps TCGA_DESC to
    tissue groups defined in config.yaml).
    """
...
def main():
    """Run Leave-One-Cell-Line-Out by tissue group."""
    print(f"╔══════════════════════════════════════════════════════════════╗")
    print(f"║  {CASE_STUDY} — LOCLO (Cell-Blind by Tissue Group)          ║")
    print(f"║  Hold out entire tissue types, test generalization          ║")
    print(f"╚══════════════════════════════════════════════════════════════╝")
    ...
    print(f"  Tissue groups ({len(unique_groups)}):")
    ...
    report = {
        ...
        "method": "Leave-One-Cell-Line-Out (LOCLO) by tissue group",
        "grouping": "tissue_type (cancer type / TCGA_DESC)",
        "rationale": ("Tests whether PTM-BDL generalizes across cancer types "
                      "for HDAC inhibitor response prediction. Cell-blind split "
                      "per Sada Del Real et al., Brief Bioinf 2026."),
        "per_group_results": results,
        "summary": summary,
        "references": [
            "Sada Del Real et al., Brief Bioinf 2026 — cell-blind evaluation",
            "Marks & Xu, J Cell Biochem 2009 — tissue-dependent SAHA response",
            "Hartl et al., Cell Reports 2024 — HDAC inhibitor proteomics",
        ],
    }
