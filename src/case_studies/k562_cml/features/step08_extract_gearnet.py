#!/usr/bin/env python3
"""
Step 08 — Extract structural embeddings for K562/CML case study.

PDB structures (from biology.py):
  1IEP — ABL kinase + Imatinib
  2GQG — ABL kinase + Dasatinib
  2HYY — ABL kinase (apo, DFG-in)
...
from src.case_studies.k562_cml.biology import ALL_PDB_STRUCTURES
...
def extract_structural_embeddings():
    """Extract structural embeddings for ABL1 PDB structures."""
    print(f"╔══════════════════════════════════════════════════════════════╗")
    ...
    for struct in ALL_PDB_STRUCTURES:
        pdb_id = struct["id"]
        ...
        if pdb_path.exists():
            ...
            print(f"    ✓ Saved: {pdb_id}_gearnet.npy {emb.shape}")
        else:
