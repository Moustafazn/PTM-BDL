...
CASE_STUDY = cfg.get("case_study", "egfr_erbb2_tki")
...
def load_gdsc_responses():
    ...
    cs_path = PROCESSED_DIR / "gdsc" / f"gdsc_{CASE_STUDY}_responses.csv"
...
def build_multimodal_dataset():
    ...
    summary = {
        ...
        "resistance_distribution": {
            "sensitive": int(
                (df_final["resistance_label"] == 0).sum()) if "resistance_label" in df_final.columns else 0,
            "resistant": int(
                (df_final["resistance_label"] == 1).sum()) if "resistance_label" in df_final.columns else 0,
        },
        "ic50_sources": {
            "gdsc": int((df_final["ic50_source"] == "gdsc").sum()) if "ic50_source" in df_final.columns else len(
                df_final),
            "literature": int(
                (df_final["ic50_source"] == "literature").sum()) if "ic50_source" in df_final.columns else 0,
        },
        "phospho_enrichment": {
