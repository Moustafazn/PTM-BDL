...
from src.ptm_bdl.evaluation.loclo import compute_loclo_metrics as _framework_loclo_metrics
...
def compute_loclo_metrics(y_true_ic50, y_pred_ic50, y_true_cls, y_prob_cls):
    """Compute Tier A metrics for a LOCLO fold. Delegates to framework."""
    # Load optimal threshold for this case study
    ...
    if _thr_path.exists():
        ...
        _resist_thr = 0.5
    return _framework_loclo_metrics(
        y_true_ic50, y_pred_ic50, y_true_cls, y_prob_cls, threshold=_resist_thr)
...
def train_loclo_fold(dataset, train_idx, test_idx, fold_name, cfg):
    ...
    for epoch in range(num_epochs):
        train_metrics = train_epoch(
            model, train_loader, optimizer, scheduler, focal_loss,
            1.0, 2.0, DEVICE,
        )
